    ...
    namespace polygon_codeforces_com
{
    statement::statement(const boost::filesystem::path &location,
        ...
        problem::statement(location, parse_versions(
            location.filename().string() + "/", config)) {}
        ...
        const boost::property_tree::ptree &config)
    {
        ...
        for (const boost::property_tree::ptree::value_type &statement_version: config)
        {
            if (statement_version.first == "<xmlattr>")
                continue;
            // format
            // TODO format -> mime_type
            std::string format =
                statement_version.second.get<std::string>("<xmlattr>.type", "unknown/unknown");
            const std::size_t sep = format.rfind('/');
            if (sep != std::string::npos)
                format = format.substr(sep + 1);

            // path
            std::string path = statement_version.second.get<std::string>("<xmlattr>.path");
    ...
    statement::version::version(
        ...
        const boost::filesystem::path &source):
            problem::statement::version(lang, format),
            m_source(source) {}
